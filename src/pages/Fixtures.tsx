    ...
    <div className="max-w-5xl mx-auto px-4 py-8 flex flex-col md:flex-row gap-8">
       <div className="md:w-64 shrink-0">
          <div className="flex items-center justify-between mb-4">
             ...
             <input 
                type="date" 
                className="bg-transparent border border-white/20 text-xs rounded-xl px-2 py-1 outline-none text-zinc-300"
             ...
             {upcomingDays.map((date, i) => {
                ...
                return (
                   <button 
                      ...
                      className={`whitespace-nowrap md:whitespace-normal text-left px-4 py-3 rounded-xl font-medium transition-all ${isSelected ? 'bg-accent-blue text-black shadow-[0_0_15px_rgba(0,209,255,0.4)]' : 'bg-white/5 hover:bg-white/10 text-zinc-400 hover:text-white'}`}
                   >
       ...
       <div className="flex-1 space-y-4">
          <div className="flex items-center justify-between mb-2">
             ...
             <span className="text-xs font-mono bg-white/10 px-3 py-1 rounded text-zinc-400">{format(selectedDate, 'MMM dd, yyyy')}</span>
          </div>

          {loading ? (
             <div className="flex flex-col gap-4">
                <span className="text-zinc-500 text-sm text-center">Loading official football data...</span>
                {[1,2,3,4,5,6].map(i => <div key={i} className="h-24 glass-panel animate-pulse rounded-xl border border-white/5"></div>)}
             </div>
          ) : fixtures.length === 0 ? (
             <div className="p-12 text-center glass-panel border border-white/5 rounded-xl bg-white/5">
                ...
                {fixtures.map((match, i) => {
                  ...
                  return (
                    <div 
                       ...
                       className="glass-panel p-4 flex flex-col md:flex-row md:items-center gap-4 group hover:bg-white/5 transition-colors border border-white/5 cursor-pointer rounded-xl hover:border-accent-blue/50"
                    >
                      <div className="flex items-center justify-between md:justify-start gap-4 md:w-1/4 pb-3 md:pb-0 border-b border-white/5 md:border-b-0">
                        <div className="flex flex-col text-left">
                           <span className="text-xl font-bold font-mono text-white">{format(dateObj, 'HH:mm')}</span>
                           <span className="text-xs text-zinc-500 font-medium uppercase tracking-widest">{format(dateObj, 'MMM dd')} - {match.fixture.status.short}</span>
                        </div>
                      ...
                      <div className="flex-1 w-full flex items-center justify-between px-2">
                         ...
                         <div className="px-4 py-1.5 mx-4 rounded-lg bg-primary-dark/50 border border-white/10 text-xs font-mono uppercase tracking-widest text-zinc-500 min-w-[50px] text-center">
                            {match.fixture.status.short === 'FT' || match.fixture.status.short === 'HT' ? (
                               <span className="font-bold text-white">{match.goals.home} - {match.goals.away}</span>
                            ) : 'VS'}
                         </div>
                      ...
                      <div className="hidden md:flex md:w-1/4 flex-col items-end justify-center">
                         <div className="flex items-center gap-2 mb-1">
                            ...
                            <img src={match.league.logo} className="w-5 h-5 object-contain opacity-50 grayscale group-hover:opacity-100 group-hover:grayscale-0 transition-all bg-white/10 p-0.5 rounded-sm" />
                         </div>
                         <span className="text-[9px] text-zinc-500 font-mono tracking-widest uppercase">{match.league.country}</span>
                      </div>
                    </div>
                  )
                })}
